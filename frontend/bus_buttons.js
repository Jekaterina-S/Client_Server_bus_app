document.addEventListener("DOMContentLoaded", () => {
    ...
    async function fetchBuses(stopId) {
        try {
            const response = await fetch(`http://localhost:3000/buses?stop=${encodeURIComponent(stopId)}`);
    ...
    async function fetchArrivalTimes(routeId, stopId, button) {
        ...
        try {
            const response = await fetch(`http://localhost:3000/arrival-times?route=${routeId}&stop=${stopId}&reference_date=${encodeURIComponent(localISOTime)}&offset=${offset}`);
